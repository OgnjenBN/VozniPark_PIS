...
toggleButton.addEventListener('click', () => {
    ...
    // Чување корисникове преференције у локалној меморији
...
function filterDrivers() {
    var nameInput, surnameInput, statusSelect, filterName, filterSurname, filterStatus, table, tr, td, i, txtValue;
    nameInput = document.getElementById("nameInput");
    surnameInput = document.getElementById("surnameInput");
    statusSelect = document.getElementById("statusSelect");
    filterName = nameInput.value.toLowerCase();
    filterSurname = surnameInput.value.toLowerCase();
    ...
    for (i = 0; i < tr.length; i++) {
        ...
        if (td) {
            ...
            var surname = td[2].textContent || td[2].innerText;
            var status = td[8].textContent || td[8].innerText; // Column index for "Status"

            if ((name.toLowerCase().indexOf(filterName) > -1 || filterName === "") &&
                (surname.toLowerCase().indexOf(filterSurname) > -1 || filterSurname === "") &&
                (status.toLowerCase().indexOf(filterStatus) > -1 || filterStatus === "")) {
                tr[i].style.display = "";
            } else {
